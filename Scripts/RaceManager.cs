...
public class RaceManager : MonoBehaviour
{
    ...
    {
        // Remove any existing car controllers
        var existingController = car.GetComponent<CarController>();
        if (existingController != null) Destroy(existingController);
        ...
        // No need to setup wheel colliders anymore since we're using direct forces
    ...
    {
        // Mute all engines at start
        if (player1Car != null)
        {
            var carController = player1Car.GetComponent<CarController>();
            if (carController != null) carController.MuteEngine(true);
            var backupController = player1Car.GetComponent<Backup>();
            if (backupController != null) backupController.MuteEngine(true);
        }
        ...
        if (player2Car != null)
        {
            if (isAIMode)
            {
                ...
                if (aiController != null) aiController.MuteEngine(true);
            }
            else
            {
                var carController = player2Car.GetComponent<CarController>();
                if (carController != null) carController.MuteEngine(true);
        ...
        // Unmute engines and start AI when race begins
        if (player1Car != null)
        {
            var carController = player1Car.GetComponent<CarController>();
            if (carController != null) carController.MuteEngine(false);
            var backupController = player1Car.GetComponent<Backup>();
            if (backupController != null) backupController.MuteEngine(false);
        }
    ...
    public void CleanupForSceneChange()
    {
        ...
        {
            var carController = player1Car.GetComponent<CarController>();
            if (carController != null) carController.StopEngine();
            var backupController = player1Car.GetComponent<Backup>();
            if (backupController != null) backupController.StopEngine();
        }
    ...
    private void ResetCarInstant(GameObject car)
    {
        ...
        if (rb != null)
        {
            ...
            Quaternion respawnRotation = checkpointManager != null
                ? checkpointManager.GetLastCheckpointRotation(car.name)
            ...
            // Ensure car is upright
            Vector3 currentUp = car.transform.up;
            if (Vector3.Dot(currentUp, Vector3.up) < 0.5f)
            {
