...
public class CheckpointManager : MonoBehaviour
{
    ...
    private void Start()
    {
        // If there are checkpoints, set the first one as starting point for all cars
        if (checkpoints.Count > 0)
        {
            // Find all cars in the scene
            CarController[] carControllers = Object.FindObjectsByType<CarController>(FindObjectsSortMode.None);
            Backup[] backupControllers = Object.FindObjectsByType<Backup>(FindObjectsSortMode.None);

            // Set initial checkpoint for CarController cars
            foreach (CarController car in carControllers)
            {
                lastCheckpoints[car.gameObject.name] = checkpoints[0];
                Debug.Log($"Set initial checkpoint for {car.gameObject.name}");
            }
